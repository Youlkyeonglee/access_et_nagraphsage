...
def train(cfg: dict):
    ...
    # nn.DataParallel 배치 분할로 GPU당 배치가 줄어드는 문제 방지 — CRAT-Pred는 파라미터 68K로
    # 5개 중 가장 작아 이 문제에 가장 취약하다(2026-07-11 실측: 4GPU 120s/ep > 1GPU 57.6s/ep).
    train_batch_size = tcfg['batch_size'] * max(n_gpus, 1)
    if n_gpus > 1:
        print(f'[CRAT-Pred-TSEM] {n_gpus}개 GPU 병렬 — GPU당 배치 {tcfg["batch_size"]} 유지 위해 '
    ...
    for epoch in range(1, tcfg['num_epochs'] + 1):
        ...
        t0 = time.time()
        total_loss = n = 0
        pbar = tqdm(train_loader, desc=f'[CRAT-Pred] Epoch {epoch:03d}', leave=False)
        for batch in pbar:
            batch_gpu = {
