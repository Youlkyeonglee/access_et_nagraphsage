"""
STGCN-adapted 학습 스크립트 — TSEM-SAGE와 동일 데이터·loss·평가로 공정 비교.

comparison/hivt_tsem, qcnet_tsem/train_*.py와 완전히 동일한 구조 — train_tsem.py의 핵심 로직을
그대로 재사용하고 모델 클래스만 STGCNTSEMAdapted로 교체.

사용법:
  cd /home/oem/TNA_research
  python comparison/stgcn_tsem/train_stgcn_tsem.py --config comparison/stgcn_tsem/config.yaml
"""
...
from model import STGCNTSEMAdapted  # noqa: E402
...
def train(cfg: dict):
    ...
    print(f'[STGCN-TSEM] CSV {len(csv_files)}개  W={W}  H={H}  stop_persist_delta={stop_delta}')
    label_dist = summarize_label_distribution(csv_files, W, H, stop_persist_delta=stop_delta)
    print('[STGCN-TSEM] train split state(t+H) 분포:', json.dumps(label_dist, indent=2))
    ...
    print(f'[STGCN-TSEM] augmentation (train split만 적용): {augment.describe()}')
    # nn.DataParallel 배치 분할로 GPU당 배치가 줄어드는 문제 방지(CRAT-Pred adapter에서 실측된
    # 소형 모델의 4GPU 오버헤드 문제와 동일한 관례 — comparison/cratpred_tsem/train_*.py 참조).
    train_batch_size = tcfg['batch_size'] * max(n_gpus, 1)
    if n_gpus > 1:
        print(f'[STGCN-TSEM] {n_gpus}개 GPU 병렬 — GPU당 배치 {tcfg["batch_size"]} 유지 위해 '
    ...
    mcfg = cfg['model']
    raw_model = STGCNTSEMAdapted(
        W=W, K=gcfg['K_max'], Kt=mcfg['Kt'], block_channels=mcfg['block_channels'],
        droprate=mcfg['droprate'], adj_sigma=mcfg['adj_sigma'], num_classes=mcfg['num_classes'],
        bias=mcfg.get('bias', True),
    ).to(device)
    print(f'모델: STGCN-adapted  params={raw_model.count_parameters():,}')
    model = raw_model
    if n_gpus > 1:
        print(f'[STGCN-TSEM] {n_gpus}개 GPU 사용 (nn.DataParallel)')
    ...
    print(f'[STGCN-TSEM] loss 구성: L_focal + {kl_weight}*KL(logits‖Uniform)'
    ...
    print(f'[STGCN-TSEM] early stopping: {"patience=" + str(patience) if patience > 0 else "미사용"}'
    ...
    for epoch in range(1, tcfg['num_epochs'] + 1):
        ...
        t0 = time.time()
        total_loss = n = 0
        pbar = tqdm(train_loader, desc=f'[STGCN] Epoch {epoch:03d}', leave=False)
        for batch in pbar:
            batch_gpu = {
        ...
        if val_m['accuracy'] > best_val:
            ...
            if patience > 0 and epochs_no_improve >= patience:
                print(f'[STGCN-TSEM] early stopping: {patience} epoch 연속 미갱신, epoch {epoch}에서 중단')
                break

    raw_model.load_state_dict(torch.load(save_dir / 'best.pt', map_location=device)['model'])
    print(f'[STGCN-TSEM] 최종 평가는 best.pt(epoch {best_epoch}, val_acc={best_val:.4f})로 수행')
    ...
    results = {
        'model': 'STGCN-adapted', 'test': test_m, 'persist_baseline_test': persist_m,
        'label_distribution_train': label_dist, 'W': W, 'H': H,
        'params': raw_model.count_parameters(),
    }
    with open(save_dir / 'results.json', 'w') as f:
        json.dump(results, f, indent=2)
    print('[STGCN-TSEM] test:', json.dumps(test_m, indent=2))
    print('[STGCN-TSEM] Persist baseline test acc:', persist_m['accuracy'])
