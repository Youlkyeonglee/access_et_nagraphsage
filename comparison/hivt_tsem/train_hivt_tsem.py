"""
HiVT-adapted 학습 스크립트 — TSEM-SAGE와 동일 데이터·loss·평가로 공정 비교.

train_tsem.py의 핵심 로직(FocalLoss, class weight, OneCycleLR, early stopping,
best.pt 기준 최종평가, Persist baseline 비교)을 그대로 재사용한다 — 데이터로더와
평가 함수(modules/*)는 import해서 그대로 쓰고, 모델 클래스만 HiVTTSEMAdapted로 교체.
TSEM-SAGE 전용 보조loss(lambda_temporal/lambda_spatial, aux head)는 HiVT 모델이
해당 출력을 만들지 않으므로 사용하지 않는다(KL·불확실성 가중치는 logits/샘플 단위라
그대로 지원).

사용법:
  cd /home/oem/TNA_research
  python comparison/hivt_tsem/train_hivt_tsem.py --config comparison/hivt_tsem/config.yaml
"""
...
from model import HiVTTSEMAdapted  # noqa: E402
...
def train(cfg: dict):
    ...
    print(f'[HiVT-TSEM] CSV {len(csv_files)}개  W={W}  H={H}  stop_persist_delta={stop_delta}')
    label_dist = summarize_label_distribution(csv_files, W, H, stop_persist_delta=stop_delta)
    print('[HiVT-TSEM] train split state(t+H) 분포:', json.dumps(label_dist, indent=2))
    ...
    print(f'[HiVT-TSEM] loss 구성: L_focal + {kl_weight}*KL(logits‖Uniform)'
    ...
    print(f'[HiVT-TSEM] early stopping: {"patience=" + str(patience) if patience > 0 else "미사용"}'
    ...
    for epoch in range(1, tcfg['num_epochs'] + 1):
        ...
        if val_m['accuracy'] > best_val:
            ...
            if patience > 0 and epochs_no_improve >= patience:
                print(f'[HiVT-TSEM] early stopping: {patience} epoch 연속 미갱신, epoch {epoch}에서 중단')
                break

    raw_model.load_state_dict(torch.load(save_dir / 'best.pt', map_location=device)['model'])
    print(f'[HiVT-TSEM] 최종 평가는 best.pt(epoch {best_epoch}, val_acc={best_val:.4f})로 수행')
    ...
    results = {
        'model': 'HiVT-adapted', 'test': test_m, 'persist_baseline_test': persist_m,
        'label_distribution_train': label_dist, 'W': W, 'H': H,
        'params': raw_model.count_parameters(),
    }
    with open(save_dir / 'results.json', 'w') as f:
        json.dump(results, f, indent=2)
    print('[HiVT-TSEM] test:', json.dumps(test_m, indent=2))
    print('[HiVT-TSEM] Persist baseline test acc:', persist_m['accuracy'])
