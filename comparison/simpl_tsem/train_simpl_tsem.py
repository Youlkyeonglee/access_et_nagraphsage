"""
SIMPL-adapted 학습 스크립트 — TSEM-SAGE와 동일 데이터·loss·평가로 공정 비교.

comparison/hivt_tsem, qcnet_tsem, cratpred_tsem/train_*.py와 완전히 동일한 구조 —
train_tsem.py의 핵심 로직을 그대로 재사용하고 모델 클래스만 SIMPLTSEMAdapted로 교체.

사용법:
  cd /home/oem/TNA_research
  python comparison/simpl_tsem/train_simpl_tsem.py --config comparison/simpl_tsem/config.yaml
"""
...
from model import SIMPLTSEMAdapted  # noqa: E402
...
def train(cfg: dict):
    ...
    print(f'[SIMPL-TSEM] CSV {len(csv_files)}개  W={W}  H={H}  stop_persist_delta={stop_delta}')
    label_dist = summarize_label_distribution(csv_files, W, H, stop_persist_delta=stop_delta)
    print('[SIMPL-TSEM] train split state(t+H) 분포:', json.dumps(label_dist, indent=2))
    ...
    mcfg = cfg['model']
    raw_model = SIMPLTSEMAdapted(
        W=W, K=gcfg['K_max'], hidden_size=mcfg['hidden_size'], n_fpn_scale=mcfg['n_fpn_scale'],
        d_rpe=mcfg['d_rpe'], n_layer=mcfg['n_layer'], n_head=mcfg['n_head'],
        dropout=mcfg['dropout'], rpe_radius=mcfg.get('rpe_radius', 100.0),
        num_classes=mcfg['num_classes'],
    ).to(device)
    print(f'모델: SIMPL-adapted  params={raw_model.count_parameters():,}')
    model = raw_model
    if n_gpus > 1:
        print(f'[SIMPL-TSEM] {n_gpus}개 GPU 사용 (nn.DataParallel)')
    ...
    print(f'[SIMPL-TSEM] loss 구성: L_focal + {kl_weight}*KL(logits‖Uniform)'
    ...
    print(f'[SIMPL-TSEM] early stopping: {"patience=" + str(patience) if patience > 0 else "미사용"}'
    ...
    for epoch in range(1, tcfg['num_epochs'] + 1):
        ...
        if val_m['accuracy'] > best_val:
            ...
            if patience > 0 and epochs_no_improve >= patience:
                print(f'[SIMPL-TSEM] early stopping: {patience} epoch 연속 미갱신, epoch {epoch}에서 중단')
                break

    raw_model.load_state_dict(torch.load(save_dir / 'best.pt', map_location=device)['model'])
    print(f'[SIMPL-TSEM] 최종 평가는 best.pt(epoch {best_epoch}, val_acc={best_val:.4f})로 수행')
    ...
    results = {
        'model': 'SIMPL-adapted', 'test': test_m, 'persist_baseline_test': persist_m,
        'label_distribution_train': label_dist, 'W': W, 'H': H,
        'params': raw_model.count_parameters(),
    }
    with open(save_dir / 'results.json', 'w') as f:
        json.dump(results, f, indent=2)
    print('[SIMPL-TSEM] test:', json.dumps(test_m, indent=2))
    print('[SIMPL-TSEM] Persist baseline test acc:', persist_m['accuracy'])
