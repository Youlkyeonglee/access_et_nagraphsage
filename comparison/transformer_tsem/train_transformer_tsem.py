"""
Transformer-adapted 학습 스크립트 — TSEM-SAGE와 동일 데이터·loss·평가로 공정 비교.

comparison/cratpred_tsem/train_cratpred_tsem.py와 완전히 동일한 구조 — train_tsem.py의 핵심 로직을
그대로 재사용하고 모델 클래스만 TransformerTSEMAdapted로 교체.

사용법:
  cd /home/oem/TNA_research
  python comparison/transformer_tsem/train_transformer_tsem.py --config comparison/transformer_tsem/config.yaml
"""
...
from model import TransformerTSEMAdapted  # noqa: E402
...
def train(cfg: dict):
    ...
    print(f'[Transformer-TSEM] CSV {len(csv_files)}개  W={W}  H={H}  stop_persist_delta={stop_delta}')
    label_dist = summarize_label_distribution(csv_files, W, H, stop_persist_delta=stop_delta)
    print('[Transformer-TSEM] train split state(t+H) 분포:', json.dumps(label_dist, indent=2))
    ...
    print(f'[Transformer-TSEM] augmentation (train split만 적용): {augment.describe()}')
    # nn.DataParallel 배치 분할로 GPU당 배치가 줄어드는 문제 방지(CRAT-Pred-adapted와 동일 이유).
    train_batch_size = tcfg['batch_size'] * max(n_gpus, 1)
    if n_gpus > 1:
        print(f'[Transformer-TSEM] {n_gpus}개 GPU 병렬 — GPU당 배치 {tcfg["batch_size"]} 유지 위해 '
    ...
    mcfg = cfg['model']
    raw_model = TransformerTSEMAdapted(
        W=W, K=gcfg['K_max'], d_model=mcfg['d_model'], nhead=mcfg['nhead'],
        num_layers=mcfg['num_layers'], dim_feedforward=mcfg['dim_feedforward'],
        dropout=mcfg.get('dropout', 0.1), num_classes=mcfg['num_classes'],
    ).to(device)
    print(f'모델: Transformer-adapted  params={raw_model.count_parameters():,}')
    model = raw_model
    if n_gpus > 1:
        print(f'[Transformer-TSEM] {n_gpus}개 GPU 사용 (nn.DataParallel)')
    ...
    print(f'[Transformer-TSEM] loss 구성: L_focal + {kl_weight}*KL(logits‖Uniform)'
    ...
    print(f'[Transformer-TSEM] early stopping: {"patience=" + str(patience) if patience > 0 else "미사용"}'
    ...
    for epoch in range(1, tcfg['num_epochs'] + 1):
        ...
        t0 = time.time()
        total_loss = n = 0
        pbar = tqdm(train_loader, desc=f'[Transformer] Epoch {epoch:03d}', leave=False)
        for batch in pbar:
            batch_gpu = {
        ...
        if val_m['accuracy'] > best_val:
            ...
            if patience > 0 and epochs_no_improve >= patience:
                print(f'[Transformer-TSEM] early stopping: {patience} epoch 연속 미갱신, epoch {epoch}에서 중단')
                break

    raw_model.load_state_dict(torch.load(save_dir / 'best.pt', map_location=device)['model'])
    print(f'[Transformer-TSEM] 최종 평가는 best.pt(epoch {best_epoch}, val_acc={best_val:.4f})로 수행')
    ...
    results = {
        'model': 'Transformer-adapted', 'test': test_m, 'persist_baseline_test': persist_m,
        'label_distribution_train': label_dist, 'W': W, 'H': H,
        'params': raw_model.count_parameters(),
    }
    with open(save_dir / 'results.json', 'w') as f:
        json.dump(results, f, indent=2)
    print('[Transformer-TSEM] test:', json.dumps(test_m, indent=2))
    print('[Transformer-TSEM] Persist baseline test acc:', persist_m['accuracy'])
