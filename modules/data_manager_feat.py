"""
ET-NAGraphSAGE Temporal Dataloader — Feature-Enhanced Version
==============================================================
data_manager.py 대비 변경점:
  노드 피처 (8D): [position_x, position_z, speed, direction_x, direction_z,
                   acceleration, vel_x, vel_z]
    vel_x = speed * direction_x  (명시적 속도 벡터)
    vel_z = speed * direction_z

  엣지 피처 (7D): [rel_speed, rel_accel, rel_dir_x, rel_dir_z, distance,
                   rel_pos_x, rel_pos_z]
    rel_pos_x/z = 이웃과의 실제 상대 위치 (방향+거리 동시 포함)
    기존에는 distance(스칼라)만 있어 좌/우 구분 불가 → LC 감지 취약

이 두 가지 추가로 LaneChange 감지에 필요한 lateral 정보를 명시적으로 제공.
"""
...
NODE_DIM   = 8   # 6 base + vel_x + vel_z
EDGE_DIM   = 7   # 5 base + rel_pos_x + rel_pos_z
    ...
    def __init__(self, csv_path: str):
        ...
        for frame, grp in df.groupby('frame'):
            ...
            for _, row in grp.iterrows():
                ...
                base     = np.array([row[c] for c in NODE_COLS], dtype=np.float32)
                vel_x    = base[2] * base[3]   # speed * direction_x
                vel_z    = base[2] * base[4]   # speed * direction_z
                node_dict[oid]  = np.concatenate([base, [vel_x, vel_z]])
...
def _compute_edge_feat(src_node: np.ndarray, dst_node: np.ndarray) -> np.ndarray:
    """
    src_node → dst_node 방향 엣지 피처 (7D).
    [rel_speed, rel_accel, rel_dir_x, rel_dir_z, distance, rel_pos_x, rel_pos_z]

    rel_pos_x/z: 이웃의 실제 상대 위치 — 부호가 있어 좌/우 구분 가능.
    기존 distance(스칼라)는 크기만 알려줬지만, 상대 위치 벡터는
    이웃이 ego 기준으로 어느 방향에 있는지(lateral vs longitudinal)를 제공.
    """
    rel_speed = dst_node[2] - src_node[2]
    rel_accel = dst_node[5] - src_node[5]
    rel_dir_x = dst_node[3] - src_node[3]
    rel_dir_z = dst_node[4] - src_node[4]
    rel_pos_x = dst_node[0] - src_node[0]
    rel_pos_z = dst_node[1] - src_node[1]
    dist      = float(np.sqrt(rel_pos_x ** 2 + rel_pos_z ** 2))
    return np.array(
        [rel_speed, rel_accel, rel_dir_x, rel_dir_z, dist, rel_pos_x, rel_pos_z],
        dtype=np.float32,
    )
