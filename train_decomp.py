...
from models.et_nagraphsage_decomp import ETNAGraphSAGEDecomp
...
def train(cfg):
    ...
    model = ETNAGraphSAGEDecomp(
        ...
        decomp_kernel=m.get("decomp_kernel",5), decomp_learnable=m.get("decomp_learnable",True)).to(device)
    print(f"[Decomp] k={m.get('decomp_kernel',5)}  파라미터: {model.count_parameters():,}개  |  {device}")
    ...
    print(f"\n{'─'*60}\n실험(Decomp): {exp_name} | T={cfg['graph']['T']}\n{'─'*60}")
...
def parse_args():
    ...
    p.add_argument('--decomp_kernel', type=int, default=None)
    p.add_argument('--decomp_learnable', type=int, default=None)
...
if __name__ == '__main__':
    ...
    if a.decomp_kernel is not None: cfg['model']['decomp_kernel'] = a.decomp_kernel
    if a.decomp_learnable is not None: cfg['model']['decomp_learnable'] = bool(a.decomp_learnable)
